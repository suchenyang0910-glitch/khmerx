    ...
    <div className="space-y-4">
      {ann.length ? (
        <Card className="p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-sm font-semibold text-zinc-900">公告</div>
              ...
              {ann[0].link_url ? (
                <a className="mt-2 inline-block text-sm text-blue-600" href={ann[0].link_url} target="_blank" rel="noreferrer">
                  查看详情 <ArrowRight className="inline h-4 w-4" />
                </a>
              ) : null}
            </div>
      ...
      <div className="rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-500 p-4 text-white shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs opacity-90">信用等级</div>
        ...
        <div className="mt-3 flex items-center justify-between rounded-2xl bg-white/15 px-3 py-2">
          <div className="text-sm">可借额度</div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link to="/borrow">
          <Button className="w-full">我要借钱</Button>
        </Link>
        <Link to="/lend">
          <Button variant="secondary" className="w-full">我要出借</Button>
        </Link>
      </div>
      ...
      <Card className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-sm font-semibold text-zinc-900">为什么你能借？</div>
            <div className="mt-1 text-sm text-zinc-600">额度与利率由信用分、历史表现与风控状态决定。</div>
          </div>
          <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-blue-50 text-blue-700">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {isNewUser ? <Badge tone="blue">新用户建议 $50-$100</Badge> : <Badge tone="green">老用户可提高额度</Badge>}
          <Badge tone="zinc">按时还款可提升信用</Badge>
          <Badge tone={(risk?.risk_level && ["flagged", "restricted", "blocked"].includes(risk.risk_level)) ? "yellow" : "green"}>
            风控：{risk?.risk_level || user?.risk_level || "normal"}
          </Badge>
        </div>
      </Card>
      ...
      <Card className="p-4">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm font-semibold text-zinc-900">借款成功率提示</div>
            <div className="mt-1 text-sm text-zinc-600">推荐金额更容易成交，首次建议 7 天。</div>
          </div>
          <Link to="/borrow" className="text-sm text-blue-600">
            去创建 <ArrowRight className="inline h-4 w-4" />
          </Link>
        </div>
      </Card>
    </div>
